...
const ProfessionalVerification = () => {
  ...
  const uploadToCloudinary = async (file) => {
    ...
    const response = await fetch("https://api.cloudinary.com/v1_1/dxs8fzo1y/image/upload", {
      method: "POST",
      body: formData,
    });
      ...
      <div className="form-sections">
        <div className="form-section left-section">
          <div className="form-group">
            ...
            <input type="number" name="experience" className="form-control" placeholder="Years" min="0" required disabled={isSubmitting} />
          </div>

          <div className="form-group">
            <label htmlFor="profession">Profession *</label>
            <select name="profession" className="form-control" required disabled={isSubmitting} onChange={(e) => setSelectedProfession(e.target.value)}>
          ...
          {selectedProfession === "Doctor" && (
            <div className="form-group">
              ...
              <select name="specialization" className="form-control" required disabled={isSubmitting}>
                <option value="">Select</option>
          ...
          <div className="form-group">
            ...
            <select name="consultationMethod" className="form-control" required disabled={isSubmitting}>
              <option value="">Select</option>
          ...
          <div className="form-group">
            ...
            <input type="file" accept="image/*" required disabled={isSubmitting} onChange={(e) => setPhotoFile(e.target.files[0])} />
          </div>

          <div className="form-group">
            <label>Medical License *</label>
            <input type="file" accept="image/*" required disabled={isSubmitting} onChange={(e) => setDocumentFile(e.target.files[0])} />
          </div>
        </div>
            ...
            <div className="days-grid">
              {daysOfWeek.map((day) => (
                <label key={day} className="day-checkbox">
                  <input type="checkbox" name="availableDays" value={day} disabled={isSubmitting} />
